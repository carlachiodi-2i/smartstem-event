...
class Agent:
    def __init__(self, number_of_games=1000, adversary='pc'):
        self.game_count = 0
    ...
    def handle_turn_user(self):
        game = self.game
        chosen_card = input('Which card would you like to play (type the name of the card or "draw"): ').lower()
        reward = game.turn.play(chosen_card)
        self.game_over = game.turn.game_over
    ...
    def reset_game(self):
        ...
        self.game.play_notifier('Player-2', self.handle_turn_ai)
        if self.adversary != 'pc':
            self.game.play_notifier('Player-1', self.handle_turn_user)
        self.game_count += 1
