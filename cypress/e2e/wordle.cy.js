...
function nextWord(wordList) {
  ...
  cy.get(`[aria-label="Row ${count}"]`)
    .find('[aria-roledescription="tile"]')
    .each(($tile, position) => {
      const letter = $tile.attr("aria-label").split(", ")[1].toLowerCase();
      // only consider characters we see for first time
      if (seen.has(letter)) {
        return;
      }
      seen.add(letter);
      const evaluation = $tile.attr("data-state");
      console.log(letter, evaluation, position);
      ...
      if (evaluation === "absent") {
        ...
        // increase countCorrectLetters
        countCorrectLetters += 1;
        wordList = wordList.filter((words) => words[position] === letter);
      }
    })
    .then(() => {
      if (countCorrectLetters === countUniqueLetters(word)) {
        cy.log("**SOLVED**");
      } else if (count >= 6) {
        cy.log("Game over!");
      } else {
...
describe("Beat Wordle", () => {
  it("Solve Wordle", () => {
    ...
    cy.visit("https://www.nytimes.com/games/wordle/index.html").wait(1000);
    cy.get('button[data-testid="Play"]').click().wait(1000);
    cy.get('[data-testid="icon-close"]').click().wait(1000);
    cy.get("#wordle-app-game").then(() => {
      nextWord(FullWordList);
    });
  });
});
